...
const openAiFetchLabel = computed(() => {
  if (openAiFetchStatus.value === "loading") return "Loading";
  if (openAiFetchStatus.value === "ok") return "Models loaded";
  if (openAiFetchStatus.value === "error") return "Load failed";
  return "Idle";
});
...
const isOpenAiProvider = computed(() => provider.value === "openai");
const isLocalProvider = computed(() => provider.value === "local" || provider.value === "ollama");
const modelOptions = computed(() => {
  let baseOptions = modelsByProvider[provider.value] ?? [];
  if (isLocalProvider.value && ollamaModels.value.length) {
    baseOptions = ollamaModels.value;
  } else if (isOpenAiProvider.value && openAiModels.value.length) {
    baseOptions = openAiModels.value;
  }
...
const useModelSelect = computed(
  () => !isOpenAiProvider.value || openAiModels.value.length > 0,
);
...
const openAiFetchTime = computed(() =>
  openAiFetchAt.value ? formatTime(openAiFetchAt.value) : "",
);
    ...
    <div class="settings-grid">
      <section class="card">
        ...
        <div class="field-grid">
          ...
          <label>
            <span>Model</span>
            <select v-if="useModelSelect" v-model="model">
